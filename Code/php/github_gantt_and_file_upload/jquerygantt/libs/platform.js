...
jQuery.fn.createErrorAlert = function(errorCode, message) {
  this.each(function() {
    ...
    if ($("#" + el.attr("id") + "error").size() <= 0) {
      ...
      err=$(err);
      err.attr("title",errMess).attr("src","/jquerygantt/res/alert.gif");
      el.after(err);
    }
  });
  return this;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  if(window.name!=iframeId)
    bg.css({backgroundImage:"url('/jquerygantt/res/black_70.png')"});
  ...
  bdiv.append("<img src=\"/jquerygantt/res/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
  bdiv.find("img:first").click( function(){
    bg.trigger("close");

  });

  bg.prepend(bdiv);*/ // Commented by Chuyen - JVB 2015-07-03
  $("body").append(bg);
}
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  ret.css({width:width, height:height,top:10});

  /*var bdiv= $("<div>").css({width:width,position:"relative",height:"0px", textAlign:"right", margin:"auto" });
  var img=$("<img src='/jquerygantt/res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
